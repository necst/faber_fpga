...
void hlsStrm2Array(hls::stream<ap_uint<PTR_WIDTH> >& srcStrm, ap_uint<PTR_WIDTH>* dstPtr, int rows, int cols) {
    accel_utils au;
    au.hlsStrm2Array<PTR_WIDTH,ROWS,COLS,NPC, COLOR_T, CH_WIDTH,TRIPCOUNT>(srcStrm,dstPtr, rows, cols);
}


template <int PTR_WIDTH, int ROWS, int COLS, int NPC, int COLOR_T, int CH_WIDTH>
void Array2hlsStrm(ap_uint<PTR_WIDTH>* srcPtr, hls::stream<ap_uint<PTR_WIDTH> >& dstStrm, int rows, int cols){
    accel_utils au;
    au.Array2hlsStrm<PTR_WIDTH,ROWS,COLS,NPC, COLOR_T, CH_WIDTH,
    ((ROWS * COLS * CH_WIDTH) / PTR_WIDTH)>(srcPtr,dstStrm, rows, cols);
}
...
void xfMat2hlsStrm(xf::cv::Mat<MAT_T, ROWS, COLS, NPC>& srcMat,
    ...
    int cols = srcMat.cols;
    int srcMat_cols_align_npc = ((srcMat.cols + (NPC - 1)) >> XF_BITSHIFT(NPC)) << XF_BITSHIFT(NPC);
    au.xfMat2hlsStrm<PTR_WIDTH, MAT_T, ROWS, COLS, NPC, ROWS*((COLS + NPC - 1) / NPC)>(srcMat, dstStrm,
                                                                                        srcMat_cols_align_npc);
